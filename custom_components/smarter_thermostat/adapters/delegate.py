...
from custom_components.smarter_thermostat.utils.helpers import round_by_step
...
async def load_adapter(self, integration, entity_id, get_name=False):
    ...
    try:
        self.adapter = await async_import_module(
            self.hass, "custom_components.smarter_thermostat.adapters." + integration
        )
        _LOGGER.debug(
            "smarter_thermostat %s: uses adapter %s for trv %s",
            self.device_name,
            integration,
            entity_id,
        )
        ...
        self.adapter = await async_import_module(
            self.hass, "custom_components.smarter_thermostat.adapters.generic"
        )
        _LOGGER.info(
            "smarter_thermostat %s: integration: %s isn't native supported, feel free to open an issue, fallback adapter %s",
            self.device_name,
            integration,
            "generic",
        )
...
async def set_temperature(self, entity_id, temperature):
    ...
    if rounded != t:
        _LOGGER.debug(
            "smarter_thermostat %s: delegate.set_temperature rounded %s -> %s (step=%s)",
    ...
    try:
        ...
        _LOGGER.warning(
            "smarter_thermostat %s: Failed to update last_temperature for entity_id %s: %s",
            getattr(self, "device_name", "unknown"),
            entity_id,
            e,
        )
    ...
    try:
        ...
        if _override_set_valve is not None:
            ...
            if ok:
                try:
                    ...
                    _LOGGER.exception(
                        "smarter_thermostat %s: Failed to set last_valve_percent or last_valve_method for %s in override",
                        getattr(self, "device_name", "unknown"),
                        entity_id,
                    )
                return True
        ...
        if valve_entity and valve_writable is True:
            ...
            try:
                ...
                _LOGGER.debug(
                    "smarter_thermostat %s: Failed to record last_valve_percent/method for %s: %s",
                    getattr(self, "device_name", "unknown"),
                    entity_id,
                    exc,
                )
        ...
        _LOGGER.debug(
            "smarter_thermostat %s: delegate.set_valve failed for %s",
            getattr(self, "device_name", "unknown"),
            entity_id,
        )
    return False
