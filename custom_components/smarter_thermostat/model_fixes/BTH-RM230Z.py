...
compatibility with the Smarter Thermostat integration.
"""
...
async def override_set_temperature(self, entity_id, temperature):
    ...
    if model == "BTH-RM230Z":
        _LOGGER.debug(
            f"smarter_thermostat {self.device_name}: TRV {entity_id} device quirk bth-rm230z for set_temperature active"
        )
        ...
        _LOGGER.debug(
            f"smarter_thermostat {self.device_name}: TRV {entity_id} device quirk bth-rm230z found hvac_modes {hvac_modes}"
        )
