...
async def load_model_quirks(self, model, entity_id):
    ...
    module_path = f"custom_components.smarter_thermostat.model_fixes.{model_sanitized}"

    try:
        self.model_quirks = await async_import_module(self.hass, module_path)
        _LOGGER.debug(
            "smarter_thermostat %s: using quirks module '%s' for model '%s' (trv %s)",
            self.device_name,
            module_path,
            model_str or "<none>",
            entity_id,
        ...
        default_module = "custom_components.smarter_thermostat.model_fixes.default"
        try:
            self.model_quirks = await async_import_module(self.hass, default_module)
            _LOGGER.debug(
                "smarter_thermostat %s: quirks module '%s' not available for model '%s' (trv %s): %s; using default",
                self.device_name,
                module_path,
                model_str or "<none>",
                entity_id,
            ...
            _LOGGER.error(
                "smarter_thermostat %s: failed to import default quirks module '%s' after error loading '%s' for model '%s' (trv %s): %s",
                self.device_name,
                default_module,
                module_path,
                model_str or "<none>",
...
def fix_local_calibration(self, entity_id, offset):
    ...
    if offset != _new_offset:
        _LOGGER.debug(
            "smarter_thermostat %s: %s - calibration offset model fix: %s to %s",
            self.device_name,
            entity_id,
            offset,
            _new_offset,
...
def fix_valve_calibration(self, entity_id, valve):
    ...
    if valve != _new_valve:
        _LOGGER.debug(
            "smarter_thermostat %s: %s - valve calibration model fix: %s to %s",
            self.device_name,
            entity_id,
            valve,
            _new_valve,
...
def fix_target_temperature_calibration(self, entity_id, temperature):
    ...
    if temperature != _new_temperature:
        _LOGGER.debug(
            "smarter_thermostat %s: %s - temperature offset model fix: %s to %s",
            self.device_name,
            entity_id,
            temperature,
            _new_temperature,
