...
async def override_set_temperature(self, entity_id, temperature):
    ...
    if model == "BTH-RM":
        _LOGGER.debug(
            f"smarter_thermostat {self.name}: TRV {entity_id} device quirk bth-rm for set_temperature active"
        )
        ...
        _LOGGER.debug(
            f"smarter_thermostat {self.name}: TRV {entity_id} device quirk bth-rm found hvac_modes {hvac_modes}"
        )
