...
def get_trv_internal_temperatures(real_trvs: dict[str, Any] | None) -> dict[str, float]:
    ...
    real_trvs : dict or None
        The real_trvs dict from SmarterThermostat, keyed by TRV entity_id.
...
def get_hvac_bt_mode(self, mode: str) -> str:
    """Return the main HVAC mode mapping for the Smarter Thermostat.
...
def heating_power_valve_position(self, entity_id):
    ...
    _LOGGER.debug(
        f"smarter_thermostat {self.device_name}: {
            entity_id
...
async def _find_lowest_battery_in_group(self, member_ids, visited=None):
    ...
    Parameters
    ----------
    self : SmarterThermostat instance
    ...
    for member_id in member_ids:
        ...
        try:
            ...
            _LOGGER.debug(
                "smarter_thermostat: non-numeric battery state '%s' for %s",
                battery_state.state,
                battery_id,
            )
            continue
...
async def get_device_model(self, entity_id: str) -> str:
    ...
    try:
        ...
        try:
            _LOGGER.debug(
                "smarter_thermostat %s: device registry -> manufacturer=%s model=%s model_id=%s name=%s identifiers=%s",
                self.device_name,
        ...
        if isinstance(dev_model_id, str) and len(dev_model_id.strip()) >= 2:
            ...
            _LOGGER.debug(
                "smarter_thermostat %s: device.model raw='%s'",
                self.device_name,
                model_str,
            )
    ...
    _LOGGER.debug(
        "smarter_thermostat %s: get_device_model(%s) selected='%s' via %s",
        self.device_name,
        entity_id,
        selected,
        source,
