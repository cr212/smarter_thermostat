"""Controlling module for Smarter Thermostat."""
...
from custom_components.smarter_thermostat.adapters.delegate import (
    get_current_offset,
    set_hvac_mode,
    set_offset,
    set_temperature,
...
from custom_components.smarter_thermostat.events.trv import convert_outbound_states
...
from custom_components.smarter_thermostat.utils.helpers import convert_to_float

_LOGGER = logging.getLogger(__name__)


class TaskManager:
    """Task manager for Smarter Thermostat."""
...
async def control_queue(self):
    ...
    ----------
    self :
            instance of smarter_thermostat
    ...
    try:
            ...
            if self.ignore_states or self.startup_running:
                ...
                if controls_to_process is not None:
                    ...
                    try:
                        ...
                        _LOGGER.exception(
                            "smarter_thermostat %s: ERROR calculating heating power",
                            self.device_name,
                        )
                    ...
                    try:
                        ...
                        _LOGGER.exception(
                            "smarter_thermostat %s: ERROR calculating heat loss",
                            self.device_name,
                        )
                    ...
                    if self.cooler_entity_id is not None:
                        try:
                            ...
                            _LOGGER.exception(
                                "smarter_thermostat %s: ERROR controlling cooler",
                                self.device_name,
                            )
                    ...
                    if result is False:
                        try:
                            ...
                            _LOGGER.debug(
                                "smarter_thermostat %s: control queue is full, discarding task",
                                self.device_name,
                            )
        ...
        _LOGGER.debug(
            "smarter_thermostat %s: control_queue task cancelled, cleaning up",
            self.device_name,
        )
        raise
    finally:
...
async def control_trv(self, heater_entity_id=None):
    ...
    async with self._temp_lock:
        ...
        try:
            ...
            _LOGGER.debug(
                "smarter_thermostat %s: hvac action recompute failed (non critical)",
    ...
    if _trv is None or _trv.state in (STATE_UNAVAILABLE, STATE_UNKNOWN):
        _LOGGER.debug(
            "smarter_thermostat %s: TRV %s is unavailable, skipping control. "
        ...
        if not isinstance(_remapped_states, dict):
            _LOGGER.debug(
                "smarter_thermostat %s: ERROR %s %s",
                self.device_name,
                heater_entity_id,
                _remapped_states,
            )
        ...
        try:
            ...
            if (
                ...
                if bal is not None:
                    ...
                    if not ok:
                        _LOGGER.debug(
                            "smarter_thermostat %s: delegate.set_valve returned False (target=%s%%, entity=%s, source=%s)",
                            self.device_name,
                            target_pct,
                            heater_entity_id,
                            _source,
                ...
                _LOGGER.debug(
                    "smarter_thermostat %s: TRV %s does not support direct valve control due to calibration type %s",
                    self.device_name,
                    heater_entity_id,
                    _calibration_type,
                )
            ...
            _LOGGER.debug(
                "smarter_thermostat %s: set_valve not applied for %s (unsupported or failed)",
                self.device_name,
                heater_entity_id,
            )
        ...
        if (
            ...
            _LOGGER.debug(
                "smarter_thermostat %s: TO TRV set_hvac_mode: %s from: %s to: %s",
                self.device_name,
                heater_entity_id,
                _trv.state,
                _new_hvac_mode,
        ...
        if (
            ...
            if _current_calibration_s is None:
                _LOGGER.error(
                    "smarter_thermostat %s: calibration fatal error %s",
                    self.device_name,
                    heater_entity_id,
                )
            ...
            if self.real_trvs[heater_entity_id][
                ...
                _LOGGER.debug(
                    "smarter_thermostat %s: TO TRV set_local_temperature_calibration: %s from: %s to: %s",
                    self.device_name,
                    heater_entity_id,
                    _old_calibration,
                    _calibration,
        ...
        if _temperature is not None and (
            ...
            if _temperature != _current_set_temperature:
                ...
                _LOGGER.debug(
                    "smarter_thermostat %s: TO TRV set_temperature: %s from: %s to: %s",
                    self.device_name,
                    heater_entity_id,
                    old,
                    _temperature,
    ...
    if (
        ...
        if (
            ...
            _LOGGER.debug(
                "smarter_thermostat %s: TRV %s calibration already at target (%s), "
...
async def check_system_mode(self, heater_entity_id=None):
    ...
    while _real_trv["hvac_mode"] != _real_trv["last_hvac_mode"]:
        if _timeout > 360:
            _LOGGER.debug(
                "smarter_thermostat %s: %s the real TRV did not respond to the system mode change",
                self.device_name,
                heater_entity_id,
            )
            _timeout = 0
...
async def check_target_temperature(self, heater_entity_id=None):
    ...
    while True:
        ...
        if _timeout == 0:
            _LOGGER.debug(
                "smarter_thermostat %s: %s / check_target_temp / _last: %s - _current: %s",
                self.device_name,
                heater_entity_id,
        ...
        if _timeout > 360:
            _LOGGER.debug(
                "smarter_thermostat %s: %s the real TRV did not respond to the target temperature change",
                self.device_name,
                heater_entity_id,
            )
            _timeout = 0
